defmodule Timeapi.Manager do
  ...
  def userworking_by_userid_workingid(userid, workingid) do
    query = from w in Workingtime,
                  where: w.id == ^workingid,
                  where: w.user == ^userid
     Repo.one!(query)
  end

  def userworking_start_end(start, endtime) do


    query = from w in Workingtime,
                    where: w.start == ^start and
                           w.end == ^endtime
